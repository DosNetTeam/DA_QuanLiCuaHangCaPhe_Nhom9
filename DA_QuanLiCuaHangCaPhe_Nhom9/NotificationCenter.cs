            ...
            try
            {
                using (var db = new DataSqlContext())
                {
                    ...
                    var unpaid = db.ThanhToans
                        .Where(tt => tt.TrangThai == "Ch?a thanh toán" && tt.MaDhNavigation.NgayLap <= DateTime.Now.AddDays(-1))
        ...
        public static List<Notification> GetAllNotifications()
        {
            ...
            try
            {
                using (var db = new DataSqlContext())
                {
                    ...
                    list.AddRange(inactive.Select(name => new Notification { Type = NotificationType.NhanVienInactive, Message = $"Nhân viên lâu không ho?t ??ng: {name}" }));
                    ...
                    list.AddRange(unpaid.Select(u => new Notification { Type = NotificationType.UnpaidInvoice, Message = $"Hóa ??n ch?a thanh toán: #{u.MaDh} - {u.NgayLap?.ToString("dd/MM/yy")}", Data = u.MaDh }));
            ...
            if (list.Count == 0)
                list.Add(new Notification { Type = NotificationType.LowStock, Message = "Không có thông báo m?i." });
